...
void find_all_gadget_from_ret(const uint8_t *data, uint64_t vaddr,
  ...
  uint32_t size_biggest_instruction =
  ...
  while (EIP < ending_instr_disasm.address) {
    ...
    for (uint32_t nb_ins = 0; nb_ins < depth; nb_ins++) {
      ...
      if (ret == UnknownInstruction || ret == OutOfBlock ||
          disass_engine.is_valid_instruction(instr) == false)
        break;
      ...
      list_of_instr.emplace_back(instr.disassembly, instr.size, instr.bytes);
    ...
    if (is_a_valid_gadget) {
      ...
      std::shared_ptr<Gadget> gadget =
          std::make_shared<Gadget>(gadget_start_address);
...
void find_rop_gadgets(
    const uint8_t *data, uint64_t size, uint64_t vaddr, uint32_t depth,
  ...
  for (uint64_t offset = 0; offset < size; offset += alignement) {
    ...
    /* OK either this is an unknow opcode & we goto the next one
     Or the instruction encountered is too long & we also goto the next one in
     that case */
    if (ret == UnknownInstruction || ret == OutOfBlock)
      continue;
    ...
    if (disass_engine.is_valid_ending_instruction(instr)) {
      ...
      only_ending_instr.emplace_back(ret_instr.disassembly, ret_instr.size,
                                     ret_instr.bytes);

      std::shared_ptr<Gadget> gadget_with_one_instr =
          std::make_shared<Gadget>(offset);
