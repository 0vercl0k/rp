...
ArmCapstone::ArmCapstone(uint32_t thumb_mode) : is_thumb(true) {
...
ArmCapstone::~ArmCapstone() { cs_close(&m_handle); }

InstructionInformation ArmCapstone::disass(const uint8_t *data, uint64_t len,
                                           uint64_t vaddr,
...
bool ArmCapstone::is_valid_ending_instruction(
