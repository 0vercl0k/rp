...
InstructionInformation IntelBeaEngine::disass(const uint8_t *data, uint64_t len,
                                              uint64_t vaddr,
  ...
  instr.virtual_address_in_memory =
      static_cast<uintptr_t>(m_disasm.VirtualAddr);
...
bool IntelBeaEngine::is_valid_ending_instruction(
    InstructionInformation &instr) const {
  /*
      Work Around, BeaEngine in x64 mode disassemble "\xDE\xDB" as an
     instruction without disassembly Btw, this is not the only case!
              XXX: BeaEngine has received a lot of recent commits recently,
     let's remove the branch to see if it's gone
  */
...
bool IntelBeaEngine::is_valid_instruction(InstructionInformation &instr) const {
  ...
  uint64_t addr_value = instr.bea_addr_value;
  /*
  Work Around, BeaEngine in x64 mode disassemble "\xDE\xDB" as an instruction
  without disassembly Btw, this is not the only case!
